package Keyword_Searcher.wordcloud.image;
...
import Keyword_Searcher.wordcloud.color.ColorCombinations;
import Keyword_Searcher.wordcloud.words.WordCount;
...
//    public static final String FONT_FAMILY = ;
    ...
    public static final String[] THEME = ColorCombinations.THEME4;
    ...
    public BufferedImage generateImage(Iterable<WordCount> words, long seed) {
        ...
        Ellipse2D.Double circle = new Ellipse2D.Double(0, 0, width + 2 * padding, height + 2 * padding);
        ...
        //Area outter = new Area(new Rectangle(0, 0, width + 2 * padding, height + 2 * padding));
        //Rectangle inner = new Rectangle(circle);
        //outter.subtract(circles);
        ...
        while (iter.hasNext()) {
            ...
            for (int i = 0; i < REJECT_COUNT * wc.n; i++) {
                ...
                if (!g.getClip().contains(s.getBounds()))
                	continue;
